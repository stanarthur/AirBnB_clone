...
class HBNBCommand(cmd.Cmd):
    ...
    def do_show(self, line):
        """Print string representation of an instance based on class and id"""
    ...
    def do_destroy(self, line):
        """Delete an instance from storage based on the class and id"""
        if not line:
            print("** class name missing **")
            return
        commands = line.split()
    ...
    def do_update(self, line):
        ...
        if commands[0] not in self.valid_classes:
            ...
            key = "{}.{}".format(commands[0],
                                 commands[1].strip('"').strip("'"))
